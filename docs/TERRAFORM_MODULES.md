...
Provisions, for RWAz: a per-app CockroachDB database, a least-privilege login
role, table grants, and (optionally) the connection string in AWS Secrets
Manager. As the object/RWA branch, RWAz owns rwa_objects writes.
...
module "helixchain" {
  ...
  app_name     = "rwaz"
  db_password  = var.rwaz_db_password   # TF_VAR_rwaz_db_password
  cluster_host = var.cluster_host
  grants = {
    rwa_objects = ["SELECT", "INSERT", "UPDATE"]   # register/transfer objects
    identities  = ["SELECT"]                       # resolve owners
  }
}
```
